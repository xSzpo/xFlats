...
class ProcessItem:
    ...
    def process_item_ototdom(self, item):
        ...
        item['price_m2'] = helpers.Scraper.digits_from_str(
            item['price_m2']) if item['price_m2'] is not None else None
        item['rooms'] = int(helpers.Scraper.digits_from_str(
            item['rooms'])) if item['rooms'] is not None else None
        item['floor_attic'] = 1 if item['floor'] == 'poddasze' else 0
        item['floor_basement'] = 1 if item['floor'] == 'suterena' else 0
        item['floor'] = helpers.Scraper.convert_floor(
            item['floor']) if isinstance(item['floor'], str) else None
...
class OutputKafka:
    ...
    def open_spider(self, spider):
        self.producer = kafka.KafkaProducer(
            bootstrap_servers=self.kafka_host+":"+self.kafka_port)
    ...
    def process_item(self, item, spider):
        ...
        logger.info("Kafka: save offer {} to topic: {}".format(
            _tmp['_id'], _tmp['producer_name']))
        return item
...
class OutputMongo:
    ...
    def __init__(self, mongo_adrress, mongo_port, mongo_dbname,
        ...
        connection = pymongo.MongoClient(
            ...
            authMechanism='SCRAM-SHA-256', serverSelectionTimeoutMS=5000)
...
class OutputFilter:
    ...
    def process_item(self, item, spider):
        ...
        elif item["price"] > 100000000:
            raise DropItem("Drop {}! - price over 100 mln pln".format(
                                                                item['_id']))
        else:
            return item
