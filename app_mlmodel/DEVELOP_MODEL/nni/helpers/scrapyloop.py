#https://gitlab.com/granitosaurus/scrapy-loop/blob/master/scrapyloop.py
import logging
import signal
...
class ScrapyLoop:
    ...
    def __init__(
            self, settings, success_interval=300, crash_interval=-1, empty_interval=5, empty_heartbeat_interval=60 * 60 * 5,
            callbacks=None, errbacks=None, empty_callbacks=None,
    ):
        """
        Scrapy crawl scheduler that loops forever.

        :settings settings: settings from get_project_settings()
        ...
        self.empty_heartbeat = LoopingCall(lambda _: logger.info('  still waiting for queue'),
    ...
    def crawl_errback(self, failure):
        ...
        if self.crash_interval < 0:
            ...
            logger.error(f'spider crashed; waiting {self.success_interval} seconds to restart, '
                      f'will restart at: {self.resume_time(self.crash_interval)}')
        return failure

    def crawl_callback(self, result):
        """Callback for when crawling finished """
        logger.info(f'spider finished; waiting {self.success_interval} seconds to restart, '
    ...
    def _crawl(self, result, crawler_or_spidercls, queue_count_func=None, **spider_kwargs):
        ...
        if queue_count_func:
            ...
            if queue_size:
                logger.info(f'found {queue_size} items in queue')
                self.waiting_for_queue = False
                if self.empty_heartbeat.running:
                    self.empty_heartbeat.stop()
            else:
                if not self.waiting_for_queue:
                    logger.info(f'spider queue is empty; will continue to check every {self.empty_interval} seconds...')
